# Cryptographie
## General
- [CyberChef](https://gchq.github.io/CyberChef/)
- [Morse Code Audio Decoder](https://databorder.com/transfer/morse-sound-receiver/)
- [substitution solver](https://www.guballa.de/substitution-solver)
- [Decoder for various ciphers](https://www.dcode.fr/en)

## RSA
- [Integer factorization calculator](https://www.alpertron.com.ar/ECM.HTM)
- [Decoder](https://www.dcode.fr/rsa-cipher)
- [Offline Decode](RSA%20Calculator.py)

# Forensic
## General
- [Code Deobfuscator](https://deobfuscate.io/)
## Images
- [All in One](https://www.aperisolve.com/)

